...
The worker has an opt-in `CALENDAR_ENABLED=true` mode, disabled by default and not enabled in the local Compose configuration yet. Each replica attempts at most one calendar operation per two-second maintenance tick. It drains pending account closes first; otherwise it advances a running simulation no sooner than five minutes after the last committed transition (or run creation for the first one). There is no catch-up loop after downtime. Both replicas must use the same flag and version. The status endpoint reports that instance's flag, current day, pending/blocked closes, nominal next transition time and simulation cadence. The nominal time is not a promise: pause, unfinished closes or safety admission can defer it.
...
The local calendar remains disabled until the transfer UI submits the current simulation day and displays close progress. The scheduler's integration tests use a newly created disposable database so expired-lease experiments cannot replace the real demo watcher evidence.
