...
The internal close executor now records the posted balance basis, exact daily interest and persisted policy in one immutable journal batch, completing its job and enqueuing delivery in the same transaction. It does not capitalize interest or change posted/reserved money. Concurrent retries return the original response. Negative live balances block the affected close with an explicit missing-product-policy reason; this does not invent an AED/BHD overdraft fee or refund policy. The fixture's fee behavior is unchanged. The `system:` command-ID prefix is reserved for internal operations and cannot be submitted through `Process`. Automatic scheduling, recurring capitalization and the calendar UI are not connected yet: the live demo still stays on its current day. The integration tests now use the real close executor rather than simulating completed job states.
