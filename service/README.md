...
External statement matching is separate from the existing internal consistency check. `GET /api/reconciliation/external?scenario=clean` compares opening settlement-asset movements at a fixed journal cutoff against an embedded synthetic CSV, independently authored rather than read back from the ledger. The optional `missing`, `duplicate`, `amount` and `date` scenarios change only the statement copy. Results include the statement identity, normalized-content SHA-256, cutoff, matched count and detailed differences. Unknown scenarios return 400. Reads have a two-second deadline and a 10,000-line ceiling.

This adapter covers only `seed-*` opening-deposit references on the two settlement assets, not later settlements or the bank as a whole. The comparison checks reference, currency, signed minor units and value day. It never nets differences away, guesses an FX match, silently deduplicates a bank statement, or produces repair postings. Persisted reconciliation runs and the dashboard workflow remain unfinished; the existing reconciliation button still performs only internal checks. The endpoint is store-tested and verified over HTTP on the rebuilt local service: forty clean matches, the four intended statement discrepancies and a 400 response for an unknown scenario.
...
`GET /api/financial` supplies complete run and UTC-today processed value, grouped by AED/BHD, alongside current customer posted/held/available balances. It uses a single PostgreSQL query snapshot and a two-second HTTP deadline. Monetary sums use `numeric` and cross the API as exact minor-unit strings. Successful transfers/splits, final captures and gross purchases count once through their positive posting legs; funding, holds, matched retries, declines, rejections, reversals and calendar work do not add processed value. This is gross forward activity, not profit or net cash flow.

The response includes 24 UTC hourly buckets, 60 minute buckets and seven UTC daily rows. Current buckets/day are partial; hourly/minute arrays are calendar-aligned, not exact rolling-window totals. Daily `commands` counts durable journal batches including maintenance, not HTTP attempts. Customer balances include only customer accounts. Empty aggregates are genuine zero values for the selected run; unavailable requests must not be rendered as zero. The full-run scan is suitable for the bounded experiment, not a precomputed warehouse. The new implementation guide explains how that read path needs to evolve with history.

The supplied v2 design is integrated into the working application, not served as a separate mock. Its money overview shows both currencies without combining them. Today/run controls change the processed-value period; pausing the monetary display retains its values and timestamp without stopping generation, journal updates or other charts. Failed refreshes retain the last snapshot with a stale label. Daily totals and the calculation explanation open as dialogs.

The chart selector switches between AED value, BHD value and command counts. Monetary charts use one-hour minute buckets, six-hour hourly buckets, or UTC-today hourly buckets; the current bucket is partial. Command counts show recorded batches and declined/rejected decisions over 10 minutes, one hour, or 24 hours, in 60 equal buckets. Counts come from one PostgreSQL query snapshot. Empty buckets mean no recorded batch, not proof that the service was healthy. Matching retries do not append another decision; these are not HTTP request, latency, or error-rate metrics. Outcome and processing-instance breakdowns use the command window and selected currency. Exact bucket values are expandable below the chart.
...
Each current ledger account is single-currency. A customer-facing multi-currency wallet could group separate AED and BHD accounts, but that grouping is not implemented here. The workspace currency selector changes account choices, command analytics and the journal preview; it never converts or combines currencies. The two-currency money overview and the monetary chart have their own labeled scopes. Current balances and operational counters are not limited by the event window. The journal preview filters the latest 60 fetched batches, so it is not a complete filtered event search. The six-day laboratory remains an explicitly separate, two-currency fixture.
...
Light, dark and system appearance settings are available in the header. Theme, sidebar and effects preferences survive reloads when browser storage is available; denied storage does not stop the application. Effects can be switched off, and reduced-motion preferences override animation. The v2 uses a native sans-serif stack, with IBM Plex Mono for selected technical values. The existing font assets and their OFL licenses remain embedded locally; there are no runtime font requests to Google. The Accounts route groups actual account classes and loads an explicitly dated snapshot of recent batches when an account expands. Its Full posted statement action opens separate, fixed-cutoff history with pagination and CSV export. The System route separates the conceptual request/commit/delivery path from observed health, source-side CDC evidence and recovery controls. Unsafe or missing admission evidence disables guarded actions; the browser cannot override the server's limits.
