...
This adapter covers only `seed-*` opening-deposit references on the two settlement assets, not later settlements or the bank as a whole. The comparison checks reference, currency, signed minor units and value day. It never nets differences away, guesses an FX match, silently deduplicates a bank statement, or produces repair postings. Persisted reconciliation runs and the dashboard workflow remain unfinished; the existing reconciliation button still performs only internal checks. The new endpoint is implemented in source and store-tested but has not yet been rebuilt into the currently running local API image.
