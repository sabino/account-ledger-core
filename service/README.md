...
External statement matching is being built separately from the existing internal consistency check. The pure comparison component matches normalized settlement-asset movements by reference and currency, checks signed minor units and value day, and reports missing entries and duplicate references explicitly. It never nets differences away, guesses an FX match, silently deduplicates a bank statement, or produces repair postings. Inputs are bounded to 10,000 lines per side. The independent statement dataset, database adapter, persisted reconciliation evidence and dashboard workflow are not connected yet; the current reconciliation button still does not establish external bank agreement.
