...
This is source-side evidence only: an active connection does not prove successful Iceberg commits, a current ClickHouse query, or a reconciled reporting cutoff. The existing retained-WAL resource guard remains separate from business approval. The isolated budget-stack [lost-slot drill](../deploy/local/lake/CDC-RECOVERY.md) passed explicit resnapshot and whole-envelope reconciliation with stricter offset validation. Default-profile integration, checkpoint-driven WAL invalidation and live lake freshness remain open work.
