...
The default `application.properties` now sets that policy after the isolated recovery drill passed. The `offset-safety.yaml` overlay retains the same explicit value for reproducibility. The separate `resnapshot.yaml` overlay temporarily changes snapshot mode to `when_needed`. Under the [versioned connector validation](https://github.com/debezium/debezium/blob/v3.6.0.Final/debezium-connector-common/src/main/java/io/debezium/connector/common/BaseSourceTask.java), an unavailable source position can then trigger a new snapshot. It does not erase the financial journal or purge old lake rows. Normal startup remains `initial`; automatic resnapshot is not enabled by default.
...
The missing batch did not appear immediately when streaming became visible: comparison initially found 1,479 of 1,480 batches and waited for the remaining lake commit. This is direct evidence that a connected slot cannot be used as proof of reporting completeness. After promoting the policy to the default local configuration, the main CDC restarted with its existing offsets. Logs confirmed `trust_offset`; complete comparison passed all 10,991 captured live envelopes and all 12 assessment envelopes. Both APIs remained healthy. This is local evidence, not a deployed-service claim.
